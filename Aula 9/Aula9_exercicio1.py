#cibersegurança
#tem que instalar a biblioteca numpy para conseguirexecutar
#execute "pip install numpy" no terminal, sem os "", e espere intalar
import numpy as np
from datetime import datetime, timedelta

def gerar_logs_acesso_array(qtd_logs, inicio, intervalo_segundos):
    # Define o tipo de dados para os registros
    dtipo = np.dtype([
    ...
    lista_ips      = np.array(["192.168.1.1", "192.168.1.2", "192.168.1.3", "10.0.0.1", "10.0.0.2"])
    lista_usuarios = np.array(["usuario_a", "usuario_b", "usuario_c", "usuario_d"])
    lista_status   = np.array(["success", "falha"])  
...
def detectar_ips_suspeitos_array(registros, limiar_falhas):
    ...
    ips_com_falhas        = registros['ip'][falhas_mask]
    ips_unicos, contagens = np.unique(ips_com_falhas, return_counts=True)
...
def principal():
    ...
    #criação do novo registro utilizado numpy.timedelta64
    ...
    print(f"Total de logs apos inserção: {len(registros)}")
